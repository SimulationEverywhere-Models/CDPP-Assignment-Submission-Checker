...
/** include files **/
#include "PlagiarismCheckerType.h"           // base header
...
/*******************************************************************
* Function Name: PlagiarismCheckerType
* Description: constructor
********************************************************************/
PlagiarismCheckerType::PlagiarismCheckerType( const string &name )
: Atomic( name )
,details(addInputPort( "details" ))
,pc_out (addOutputPort( "pc_out" ))
,pc_time(0,0,5,0)
...
// , in(addInputPort( "in" ))
{
	string time( MainSimulator::Instance().getParameter( description(), "pc_time" ) ) ;
...
********************************************************************/
Model &PlagiarismCheckerType::initFunction()
{
...
********************************************************************/
Model &PlagiarismCheckerType::externalFunction( const ExternalMessage &msg )
{
	...
	// }
	if(msg.port() == details) {
			p_local = msg.value()/10 ;
			plag_var = p_local%100 ;
			if(plag_var >= 10)
				pc_var = 1;
			else
				pc_var = 0;
			holdIn( active, pc_time );
...
********************************************************************/
Model &PlagiarismCheckerType::internalFunction( const InternalMessage & )
{
...
********************************************************************/
Model &PlagiarismCheckerType::outputFunction( const InternalMessage &msg )
{
	//TODO: implement the output function here
	// remember you can use sendOutput(time, outputPort, value) function.
	// sendOutput( msg.time(), out, 1) ;
	sendOutput( msg.time(), pc_out, pc_var) ;
	return *this;

}

PlagiarismCheckerType::~PlagiarismCheckerType()
{
